...
`examples/notes` is a complete end-to-end example: a notes server declared with `mcp-tada-server`, its snapshot generated by the CLI, and a typed client that spawns the server over stdio and narrows on `isError`.

```sh
pnpm --filter @mcp-tada/example-notes start
```
