...
| `introspect [alias]` | connects, pages `tools/list`, `prompts/list`, `resources/list`, and `resources/templates/list` (each when the capability is declared), writes the snapshot | connection or timeout failure, naming the target |
